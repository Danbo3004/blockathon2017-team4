...
module.exports =  {
   ...
   tokenName : "VNDT",
   lendingAddress : "0xb8c0a0fF713C27032aaf10B9b14108014a51BaCF",
    ...
    async.auto({
      ...
      send: ["getLoan","getNumberOfToken","getBlockNumber",function(data,callback) {
        ...
        * */
        ...
        if(data["getLoan"][1]/data["getLoan"][2] < rate){
          var error = new Error();
          error.message = "Upper";
          error.statusCode = 400;
          error.status = 400;
        ...
        if(data["getLoan"][1]/data["getLoan"][2] == rate && data["getLoan"][7].length == 42){
          var error = new Error();
          error.message = "Upper";
          error.statusCode = 400;
          error.status = 400;
        ...
        app.models.util.sendMethod( tokenAddress,tokenName,"transferBidLending",[lendingAddress,data["checkInvoice"],rate*1000,1000,_to],req,callback);
      }
      ]
    },function (err,result) {
      if(err){
   ...
   payDebt : function( req,cb){
    async.auto({
      ...
      checkInvoice:["getUser",function(data,callback){
        app.models.util.callMethod(this.lendingAddress,this.lendingName,"calInvoice",[data["getUser"].address],callback);
      }],
      getNumberOfToken : ["getUser",function (data,callback) {
        app.models.util.checkTokenBalance(data["getUser"].address,callback);
      }],
      send: ["checkInvoice","getNumberOfToken",function(data,callback) {
        ...
        app.models.util.sendMethod( this.tokenAddress,this.tokenName,"transferPayDebt",[this.lendingAddress,data["checkInvoice"]],req,callback);
      }
      ]
    },function (err,result) {
      if(err){
